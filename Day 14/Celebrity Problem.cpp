...
// Method 2:  Time complexity: O(N*N) Space Complexity: O(N) 
// Only check the inndegree of each element if it is equal to n-1 then it maybe the possible answer
// Hence take that element and check if it doesn't know anyone as we have checked that it is known by everyone
...
int celebrity(vector<vector<int> >& arr, int n) 
    {
        ...
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++){
                if(arr[i][j]){
                    inn[j]++;
                    if(inn[j]==n-1) // j is known to everyone
                        ans=j;
                }
            }
        }
       // checking if j doesn't know anyoe or not
       if(ans==-1)
       return ans;
        for(int i=0;i<n;i++){
            if(arr[ans][i]){
...
// Method 3: Time complexity: O(N) Space Complexity: O(1) Using stack
...
// Method 4: Time Complexity: O(N) Space Complexity: O(1) Two Pointer approach
